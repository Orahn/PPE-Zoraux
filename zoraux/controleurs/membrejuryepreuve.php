...
class Zoraux_Controleurs_MembreJuryEpreuve {
    ...
    function formMembreJuryEpreuve(){
        ...
        $tableMembreJuryEpreuve=new Zoraux_Modeles_MembreJuryEpreuve();
        if(isset($_GET['id'])){
            //Si l'id est recuperable depuis la methode GET, il s'agit d'une edition
            //Il faut alors recuperer l'enregistrement correspondant
            $membreJuryEpreuve=$tableMembreJuryEpreuve->get($_GET['id']);
        }else{
            //Sinon, c'est qu'il s'agit d'un nouvel enregistrement
            $membreJuryEpreuve=$tableMembreJuryEpreuve->newMembreJuryEpreuve();
        }
    ...
    function enregistrerMembreJuryEpreuve(){
        ...
        $this->informations();
        $tableMembreJuryEpreuve=new Zoraux_Modeles_MembreJuryEpreuve();
        $tableMembreJuryEpreuve->supprimerMembreJuryEpreuve($_POST['epreuve_id']);
